...
const FaFacebook = dynamic(() =>
  import("react-icons/fa").then((module) => module.FaFacebook)
);
const FaInstagram = dynamic(() =>
  import("react-icons/fa").then((module) => module.FaInstagram)
);
const FaLinkedin = dynamic(() =>
  import("react-icons/fa").then((module) => module.FaLinkedin)
);
...
const Footer = ({ currentPageData }) => {
  ...
  const FootHeader = ({ title }) => (
    <>
      <h6 className=" text-light font-b mb-0 " style={{ fontSize: 22 }}>
        {title}
      </h6>
  ...
  return (
    <footer
      className="mt-5 text-lg-start text-center bg-dark "
    ...
    >
      ...
      <section className="p-2 px-lg-5 pt-4">
        <div className="row align-items-center">
          ...
          <div className="col-lg-8 col-md-8 col-12 row m-0 text-center text-md-start">
            {[
              ...
              <div
                key={index}
                className={
                  linkDetails.title === "Work With Me"
                    ? "col-lg col-md col-sm-4 mt-lg-0 mt-2 text-center"
                    : `col mt-lg-0 mt-2 text-center`
                }
                style={{ maxWidth: `${linkDetails.title.length * 9 + 20}px` }}
              >
                <Link className="elementor-icon" href={linkDetails.link}>
                  {linkDetails.title}
                </Link>
              </div>
            ))}
          ...
          <div className="col-lg-4 col-md-4 col-12 text-lg-end text-center mt-lg-0 mt-md-0 mt-4">
            ...
            ].map((iconDetails, index) => (
              <Link
                key={index}
                href={iconDetails.link}
                name={iconDetails.name}
                aria-label="Social Icons"
                className="elementor-icon hover-dark me-4"
                target={"_blank"}
              >
                {iconDetails.title}
              </Link>
          ...
          <div className="row">
            ...
            <div className="col-sm-10 col-md-12 col-lg-4 mb-4">
              ...
              <div
                dangerouslySetInnerHTML={{
                  __html: currentPageData.siteOption.siteOptions.siteAbout,
                }}
              />
            </div>
            {/* Grid column */}
            ...
            <div className="col-sm-10 col-md-12 col-lg-4 mb-4">
              ...
              <p className="text-light mb-0">
                {currentPageData.siteOption.siteOptions.address}
              </p>
              <Link
              ...
              >
                {currentPageData.siteOption.siteOptions.email}
              </Link>
              <h4 className={"mb-0 mt-3"}>Stay Ahead In SEO: Subscribe Now</h4>
              <p>
                Get the latest SEO tips and updates straight to your inbox.
                Subscribe to our newsletter and never miss out on valuable
                insights.
              </p>
      ...
      >
        © Copyright {new Date().getFullYear()} Vikash Kumar Pal. All rights
        reserved.
        <Link
          href={"/terms-and-conditions"}
          className="elementor-icon ms-2 me-2"
        >
          Terms
        </Link>
        |
        <Link href={"/privacy-policy"} className="elementor-icon ms-2 me-2">
          {" "}
          Privacy Policy
        </Link>
      </div>
